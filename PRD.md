...
- Authentication (post-PoC): see `docs/superpowers/specs/2026-08-05-openapi-auth-design.md` and `docs/superpowers/plans/2026-08-05-openapi-auth.md`. v1 targets OpenAPI `apiKey` (header) + HTTP bearer via Apical header validation and a hexagonal `Authenticator`/`Principal`. Deferred beyond that: OAuth/OIDC flows, mutualTLS, scope-based 403, `plugin-auth` extraction, SST authorizers.
