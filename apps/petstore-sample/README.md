...
tests that prove Hexkit can generate a real **Rich Pet + Order + User** app, typecheck
...
`openapi.poc.yaml` is a Petstore-shaped **Rich Pet** plus Order and User: Pet requires
...
references Pet with a scalar `petId` foreign key. User persists on `users` with
identity `id` and lookup by `username`. `GET /pet/{petId}` requires
