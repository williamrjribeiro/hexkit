...
describe("@hexkit/plugin-next", () => {
  ...
  describe("Given ContractArtifact + ApplicationArtifact for Petstore", () => {
    ...
    it("when surface is routes, then routes and server-access are derived with empty uiPages", () => {
      ...
      expect(files.some((file) => file.path === "src/adapters/http-next/controllers.ts")).toBe(
        true,
      );
    ...
    it("when surface is routes, then generation emits handlers and server-access without resource pages", async () => {
      ...
      expect(paths.some((path) => path.endsWith("/page.tsx") && path !== "app/page.tsx")).toBe(
        false,
      );
  ...
  describe("Given generated Next App Router UI", () => {
    ...
    it("when hub pages list dynamic UI routes, then they do not emit html links to bracket paths", async () => {
      ...
      expect(hub).toContain('import Link from "next/link"');
      expect(hub).toContain('href="/ui/pet/findByStatus"');
