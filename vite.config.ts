...
export default defineConfig({
  ...
  run: {
    cache: true,
    tasks: {
      // Uncached Vite+ tasks inherit caller env. The task schema rejects `env`
      // together with `cache: false`. Controlled proofs cover PETSTORE_API_URL /
      // PETSTORE_NEXT_URL, HEXKIT_KEEP_STACK, HEXKIT_SKIP_COMPOSE, and
      // HEXKIT_DOGFOOD_OUTPUT (override). Default output dirs are stable under
      // /tmp so :down can find Compose. HEXKIT_KEEP_STACK default is still 0.
